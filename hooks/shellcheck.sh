...
parse_arguments() {
	while (($# > 0)); do
		...
		--enable)
			...
			;;
		--extra-options)
			extra_options_list="${extra_options_list} ${VALUE}"
			;;
		-*)
	...
	extra_options_list="${extra_options_list## }" # remove preceeding space
}
...
for FILE in ${files}; do
	...
	if (head -1 "${FILE}" | grep "${SHEBANG_REGEX}" >/dev/null); then
		if ! shellcheck ${enable_list:+ --enable="${enable_list}"} "${extra_options_list}" "$FILE"; then
			exit_status=1
		fi
